                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <!-- Imagen -->
                            <div class="md:col-span-2">
                                <x-input-label for="img" :value="__('Imagen (opcional)')" />
                            ...
                            <!-- Organización (Solo mostrar, no editar porque será automática) -->
                            <div class="mb-4">
                                <label for="organizations_id" class="block text-sm font-medium text-gray-700">{{ __('Organización') }}</label>
                                <input type="text" value="{{ Auth::user()->organization->name }}" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" disabled>
                                <p class="mt-1 text-sm text-gray-500">{{ __('Este evento se asociará automáticamente a tu organización.') }}</p>
                            </div>
                        </div>
                        
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('events.index') }}" class="text-gray-600 hover:text-gray-900 mr-4">
                                {{ __('Cancelar') }}
                            </a>
                            <x-primary-button>
                                {{ __('Crear Evento') }}
