    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Eventos de') }}: {{ $supervisedUser->user_name }}
            </h2>
            <div class="flex space-x-2">
                <a href="{{ route('supervised-users.show', $supervisedUser) }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
                    {{ __('Volver al Perfil') }}
                </a>
                @if($isDirectSupervisedUser)
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 ">
            ...
            <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4" role="alert">
                <p>{{ __('Este usuario es un supervisado indirecto. Solo puedes ver sus eventos, no editarlos.') }}</p>
            </div>
            @endif
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($savedFilters as $name => $filter)
                                <div class="flex items-center">
                                    <a href="{{ route('supervised-users.events', ['supervisedUser' => $supervisedUser->id, 'apply_filter' => $name]) }}" 
                                    ...
                                    <a href="{{ route('supervised-users.events', ['supervisedUser' => $supervisedUser->id, 'delete_filter' => $name]) }}" 
                    ...
                    <form action="{{ route('supervised-users.events', $supervisedUser) }}" method="GET">
                        ...
                        <div class="mt-4 flex justify-end">
                            ...
                            @if(request()->anyFilled(['search', 'category', 'date_from', 'date_to', 'location', 'min_participants', 'max_participants']))
                                <a href="{{ route('supervised-users.events', $supervisedUser) }}" class="ml-3 px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="flex flex-wrap gap-6">
                            ...
                            <div>
                                ...
                                <select id="per-page-select" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="10" {{ (Auth::user()->preferences['supervisedUserEvents_per_page'] ?? 10) == 10 ? 'selected' : '' }}>10</option>
                                    <option value="25" {{ (Auth::user()->preferences['supervisedUserEvents_per_page'] ?? 10) == 25 ? 'selected' : '' }}>25</option>
                                    <option value="50" {{ (Auth::user()->preferences['supervisedUserEvents_per_page'] ?? 10) == 50 ? 'selected' : '' }}>50</option>
                                    <option value="100" {{ (Auth::user()->preferences['supervisedUserEvents_per_page'] ?? 10) == 100 ? 'selected' : '' }}>100</option>
                                </select>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Vista de lista -->
            <div id="list-view" class="{{ (Auth::user()->preferences['supervisedUserEvents_view_type'] ?? 'card') === 'list' ? 'block' : 'hidden' }}">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200 overflow-x-auto">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($events as $event)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($event->start_date)->format('d/m/Y H:i') }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('events.show', $event) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('Ver') }}</a>
                                                @if($isDirectSupervisedUser)
                                                <a href="{{ route('events.edit', $event) }}" class="text-yellow-600 hover:text-yellow-900">{{ __('Editar') }}</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        @else
                            <p class="text-gray-500">{{ __('Este usuario no tiene eventos creados aún.') }}</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <!-- Vista de tarjetas -->
            <div id="card-view" class="{{ (Auth::user()->preferences['supervisedUserEvents_view_type'] ?? 'card') === 'card' ? 'block' : 'hidden' }}">
                @if ($events->count() > 0)
                        ...
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                            ...
                            <div class="p-4">
                                <div class="mb-2">
                                    ...
                                    <span class="text-sm font-medium">{{ \Carbon\Carbon::parse($event->start_date)->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="mb-2">
                                ...
                                <div class="mb-2">
                                    <span class="text-sm text-gray-500">{{ __('Categoría') }}:</span>
                                    <span class="text-sm font-medium">
                                    @if($event->categories->count() > 0)
                                        {{ $event->categories->first()->description }}
                                    @else
                                        {{ __('Sin categoría') }}
                                    @endif
                                    </span>
                                ...
                                <div class="flex justify-between">
                                    <a href="{{ route('events.show', $event) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('Ver Detalles') }}</a>
                                    
                                    @if($isDirectSupervisedUser)
                                    <a href="{{ route('events.edit', $event) }}" class="text-indigo-600 hover:text-indigo-800">{{ __('Editar') }}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateViewType(viewType) {
                fetch('{{ route("user-preferences.save") }}', {
                    ...
                    body: JSON.stringify({
                        key: 'supervisedUserEvents_view_type',
                        value: viewType
                    })
                }).then(response => {
                    if (response.ok) {
            ...
            function updatePerPage(perPage) {
                fetch('{{ route("user-preferences.save") }}', {
                    ...
                    body: JSON.stringify({
                        key: 'supervisedUserEvents_per_page',
                        value: perPage
                    })
                }).then(response => {
                    if (response.ok) {
